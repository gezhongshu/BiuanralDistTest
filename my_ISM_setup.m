function [SetupStruc] = my_ISM_setup(fs, c, t60, dims, p_s, p_r)
...
SetupStruc.room = dims;        % room dimensions in m

SetupStruc.mic_pos = dims/3;    % [x y z] positions of a microphone array in m
if nargin > 5
    SetupStruc.mic_pos = p_r;
end

SetupStruc.src_traj = dims/3 + [dims(1)/3, 0, 0];   % [x y z] positions of source trajectory in m. 
if nargin > 4
    SetupStruc.src_traj = p_s;
end
